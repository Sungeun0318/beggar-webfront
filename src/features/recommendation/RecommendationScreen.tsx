...
export function RecommendationScreen() {
  ...
  const budgetLabel =
    recommendationBudget == null
      ? "?®Ï? ?àÏÇ∞ Í∏∞Ï? Ï∂îÏ≤ú"
      : `1??Ï∂îÏ≤ú ?àÏÇ∞ ${money(recommendationBudget)}??

  return (
    <PhoneFrame>
      <main className="relative min-h-[852px] bg-bg">
        <AppHeaderTitled title="?àÏÇ∞??ÎßûÎäî Ï∂îÏ≤ú" onBack={() => navigate(-1)} />
        <section className="px-pageH pt-2">
          ...
          {result?.budgetGuide && (
            ...
            >
              {result.fallbackApplied ? "?Ï≤?Ï∂îÏ≤ú?¥Ïóê?? " : ""}
              {result.budgetGuide}
            </div>
          )}
              ...
              return (
                <div key={place.storeId ?? place.name} className="mb-3.5">
                  <RecommendationCard
                    ...
                    tag={place.category}
                    title={place.name}
                    walk={`${place.walkTime ?? "?ÑÎ≥¥ ?ïÎ≥¥ ?ÜÏùå"} ¬∑ ${place.address}`}
                    rating={`${place.menuName ?? "???Î©îÎâ¥"} ¬∑ ??${
          ...
          >
            ...
            <div className="ml-3 min-w-0 flex-1">
              <p className="text-[15px] font-extrabold text-text">
                ?®Îäî ?àÏÇ∞ÍπåÏ? Í≥†Î†§??Ï°∞Ìï© Ï∂îÏ≤ú
              </p>
              <p className="mt-1 text-xs font-semibold leading-[1.45] text-sub">
                Ï∞©ÌïúÍ∞Í≤©ÏóÖ?åÏ? ?®Î? ?àÏÇ∞???®Íªò Î≥¥Í≥† Ï∂îÏ≤ú?àÏñ¥.
              </p>
            </div>
          </div>
          <div className="h-6" />
...
function LocationSheet({
  ...
  const search = async () => {
    ...
    try {
      ...
      setError("Ïß??Í≤??âÏùÑ Î∂àÎü¨?§Ï? Î™ªÌñà??")
    } finally {
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center px-6">
      ...
      <div className="relative w-full max-w-[390px] rounded-card bg-white px-5 pb-6 pt-[22px]">
        <div className="flex items-center">
          <p className="flex-1 text-lg font-extrabold text-text">Ï∂îÏ≤ú Ïß??/p>
        ...
        >
          <LocateFixed size={18} /> ?ÑÏû¨ ?ÑÏπò ?¨Ïö©
        </button>
        ...
        <div className="mt-3">
          {isSearching ? (
            <p className="py-6 text-center text-sm font-semibold text-sub">Í≤??Ï§ë‚?</p>
          ) : error ? (
            <LocationMessage message={error} actionLabel="?§Ïãú Í≤???" onAction={() => void search()} />
          ) : searched && results.length === 0 ? (
            <LocationMessage message="Í≤?? Í≤∞Í≥ºÍ∞ ?ÜÏñ¥." actionLabel="?ÖÎ†•??Ïß??úºÎ°?Í≤???" onAction={() => onManualRegion(query)} />
          ) : results.length > 0 ? (
